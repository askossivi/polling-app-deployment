            ...
            script {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    withCredentials([usernamePassword(credentialsId: 'github', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                        ...
                        sh "sed -i 's+devtraining/server-app.*+devtraining/server-app:${DOCKERTAG}+g' deployments/polling-app-server.yaml"
                        sh "sed -i 's+devtraining/client-app.*+devtraining/client-app:${DOCKERTAG}+g' deployments/polling-app-client.yaml"
                        sh "cat deployments/mysql-deployment.yaml "
                        sh "cat deployments/polling-app-server.yaml"
                        sh "cat deployments/polling-app-client.yaml"
                        sh "git add ."
